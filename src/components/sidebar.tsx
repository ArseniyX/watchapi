  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent>
        <DialogHeader>
          <DialogTitle>
            Delete {itemType === "collection" ? "Collection" : "Endpoint"}
          </DialogTitle>
          <DialogDescription>
...
function CollectionTree({
  ...
  const handleDeleteClick = (item: CollectionTreeItem, e: React.MouseEvent) => {
    e.stopPropagation();
    setDeleteDialog({
      open: true,
      itemId: item.id,
  ...
  return (
    <>
      <DeleteConfirmDialog
        ...
        isDeleting={
          deleteCollectionMutation.isPending || deleteEndpointMutation.isPending
        }
      />
      {items.map((item) => (
        <div key={item.id}>
            ...
            {item.type === "folder" ? (
              ...
              <span
                className={cn(
                  ...
                  item.method === "POST" && "bg-yellow-500/20 text-yellow-400",
            ...
            {hoveredItem === item.id && (
              <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                <button
                  ...
                  title={`Delete ${
                    item.type === "folder" ? "collection" : "endpoint"
                  }`}
                >
