...
export const createApiEndpointSchema = z.object({
    ...
    timeout: z.number().int().positive("Timeout must be greater than 0").default(30000),
    interval: z.number().int().positive("Interval must be greater than 0").default(300000), // 5 minutes
...
export const updateApiEndpointSchema = z.object({
    ...
    url: z.string().url("Invalid URL format").min(1, "URL cannot be empty").optional(),
    method: z.nativeEnum(HttpMethod).optional(),
    headers: z.record(z.string(), z.string()).optional(),
    body: z.string().optional(),
    expectedStatus: z.number().int().min(100).max(599).optional(),
    timeout: z.number().int().positive("Timeout must be greater than 0").optional(),
    interval: z.number().int().positive("Interval must be greater than 0").optional(),
