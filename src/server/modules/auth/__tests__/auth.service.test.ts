...
describe('AuthService', () => {
  ...
  describe('login', () => {
    it('should login user with correct credentials', async () => {
      ...
      expect(mockUserService.verifyPassword).toHaveBeenCalledWith(mockUser, input.password)
      expect(result.user).toEqual(mockUser)
      expect(result.tokens).toHaveProperty('accessToken')
      expect(result.tokens).toHaveProperty('refreshToken')
    })
  ...
  describe('refreshToken', () => {
    it('should refresh token successfully', async () => {
      ...
      ;(jwt.verify as any).mockReturnValue({ userId: 'user-1', type: 'refresh' })
      mockUserService.getUserById = vi.fn().mockResolvedValue(mockUser)
    ...
    it('should throw error for invalid token type', async () => {
      ...
      await expect(
        service.refreshToken('invalid-type-token')
  ...
  describe('generateTokens', () => {
    it('should generate access and refresh tokens', async () => {
      ...
      mockUserService.createUser.mockResolvedValue(mockUser)
      ;(jwt.sign as any).mockReturnValueOnce('access-token').mockReturnValueOnce('refresh-token')

      const result = await service.register({ email: 'test@example.com', password: 'password' })

      expect(jwt.sign).toHaveBeenCalledWith(
        { userId: expect.any(String), email: expect.any(String), role: expect.any(String) },
        jwtSecret,
        { expiresIn: '7d' }
      )
      expect(jwt.sign).toHaveBeenCalledWith(
