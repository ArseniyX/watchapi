...
export class MonitoringService {
  ...
  async checkApiEndpoint(apiEndpointId: string): Promise<MonitoringCheckResult> {
    const endpoint = await this.apiEndpointRepository.findById(apiEndpointId)
    if (!endpoint) {
    ...
    try {
      ...
      const result: MonitoringCheckResult = {
        status:
          (error as Error).name === 'AbortError' ? CheckStatus.TIMEOUT : CheckStatus.ERROR,
        responseTime:
          (error as Error).name === 'AbortError' ? endpoint.timeout : responseTime,
        errorMessage,
      }
  ...
  private async sendAlert(endpoint: ApiEndpoint, result: MonitoringCheckResult): Promise<void> {
    const now = Date.now()
    ...
    const user = await this.monitoringRepository.findUserById(endpoint.userId)
    if (!user || !user.email) {
  ...
  async getMonitoringHistory(apiEndpointId: string, options: { skip?: number; take?: number } = {}) {
  ...
  async getAnalytics(userId: string, days: number = 7) {
    ...
    return {
      ...
      changes: {
        totalChecks: calculateChange(currentStats.totalChecks, previousStats.totalChecks),
        avgResponseTime: calculateChange(
          currentStats.avgResponseTime,
          previousStats.avgResponseTime
        ),
        errorRate: calculateChange(currentStats.errorRate, previousStats.errorRate),
  ...
  async getResponseTimeChart(userId: string, days: number = 7) {
    ...
    return this.monitoringRepository.getResponseTimeHistory(userId, from, to)
  }
